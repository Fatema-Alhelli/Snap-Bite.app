...
def find_recipe_image(recipe_id):
    ...
    for ext in ("*.jpg", "*.jpeg", "*.png", "*.webp", "*.JPG", "*.JPEG", "*.PNG", "*.WEBP"):
        matches.extend(RECIPE_IMAGES.glob(f"{recipe_id:03d}_*{ext[1:] if ext.startswith('*') else ext}"))
    # Fallback for exact numeric prefix regardless of punctuation/extension.
    if not matches:
        matches = [p for p in RECIPE_IMAGES.iterdir() if p.is_file() and p.stem.startswith(f"{recipe_id:03d}_")]
...
if page == "🏠 Welcome":
    ...
    with left:
        st.markdown(
            ...
            <div class="welcome-left">
                ...
                <div style="font-size:1.35rem;font-weight:700;">Where cooking creativity begins.</div>
    ...
    with right:
        st.markdown('<div class="welcome-right">', unsafe_allow_html=True)
        st.image(str(ASSETS / "snap_bite_intro.gif"), use_container_width=True)
        st.markdown("</div>", unsafe_allow_html=True)
    st.write("")
    if st.button("🍳 Start to Cook", use_container_width=True):
        st.session_state.nav = "📸 Snap / Upload"
        st.rerun()
    ...
    if selected:
        ...
        if detected:
            ...
            st.warning("No ingredients were detected. You can correct the result manually below.")
