...
def generate_recipe_with_llm(ingredients=None, user_prompt=""):
    ...
    ingredient_text = ", ".join(ingredients or []) or "No ingredients detected; use the ingredients mentioned by the user."
    request_text = user_prompt.strip() or "Create a recipe using the detected ingredients."
    prompt = f"""
You are Snap-Bite, a cooking assistant.
Detected ingredients: {ingredient_text}
User request: {request_text}

Create ONE realistic recipe. The detected ingredients should be the main ingredients.
Do not use pork or pork meat. Pantry basics are allowed.
Return exactly:
RECIPE NAME:
CATEGORY:
CUISINE:
...
Under INGREDIENTS, give quantities. Under INSTRUCTIONS, give numbered steps.
...
def render_generated_recipe(text):
    ...
    st.markdown(f'<div class="section-title">{data.get("recipe_name", "Your Recipe")} 🍴</div>', unsafe_allow_html=True)
    tags = []
    for key, cls, icon in [("category", "tag-b", ""), ("cuisine", "tag-g", ""), ("calories", "tag-o", "🔥 "), ("prep_time", "tag-y", "⏱️ ")]:
        if data.get(key):
    ...
    with left:
        ...
        if img:
            ...
            st.markdown('<div class="card" style="font-size:6rem;text-align:center;padding:2rem">🍽️</div>', unsafe_allow_html=True)
        st.markdown("### 👥 Servings")
        st.write(data.get("servings", "1"))
        st.markdown("### 🥕 Ingredients")
        for ing in data.get("ingredients", []):
            st.markdown(f'<div class="ingredient-row">{ing}</div>', unsafe_allow_html=True)
    with right:
        n1, n2, n3, n4 = st.columns(4)
        metrics = [(n1,"🔥","Calories","calories"),(n2,"💪","Protein","protein"),(n3,"🍚","Carbs","carbs"),(n4,"🥑","Fat","fat")]
        for col, icon, label, key in metrics:
            with col:
                st.markdown(f'<div class="metric-card"><div class="metric-num">{data.get(key,"—")}</div><div class="metric-label">{icon} {label}</div></div>', unsafe_allow_html=True)
        if data.get("description"):
            st.markdown("### About this recipe")
            st.write(data["description"])
        st.markdown("### 👨‍🍳 Instructions")
        for i, step in enumerate(data.get("instructions", []), 1):
            st.markdown(f'<div class="step-row"><b>{i}.</b> {step}</div>', unsafe_allow_html=True)
...
st.markdown("""
<style>
div[data-testid="stPopover"] { position: fixed; right: 24px; bottom: 24px; z-index: 9999; }
div[data-testid="stPopover"] > button { background: #1E6D4A !important; color: white !important; border: 0 !important; border-radius: 18px !important; font-weight: 800 !important; min-height: 3rem !important; box-shadow: 0 8px 22px rgba(5,64,35,.22) !important; }
</style>
""", unsafe_allow_html=True)
with st.popover("🤖 Recipe Bot"):
    st.markdown("### 🤖 Snap-Bite Recipe Bot")
    st.caption("Tell me your ingredients or what you want to cook.")
    bot_prompt = st.text_area("Your cooking request", placeholder="e.g. chicken, tomato and onion — make it healthy", height=90, key="bot_prompt")
    if st.session_state.detected_ingredients:
        st.caption("Detected: " + ", ".join(st.session_state.detected_ingredients))
    if st.button("🍳 Create Recipe", use_container_width=True, key="bot_create"):
        if not bot_prompt.strip() and not st.session_state.detected_ingredients:
            st.warning("Write a request or detect ingredients from an image first.")
        else:
            try:
                with st.spinner("Creating your recipe..."):
                    st.session_state.generated_recipe = generate_recipe_with_llm(st.session_state.detected_ingredients, bot_prompt)
...
if page == "🏠 Welcome":
    ...
    if selected:
        ...
        if selected_ingredients:
            ...
            left_action, right_action = st.columns([4.5, 1.2], gap="large")
            with left_action:
                st.caption("Use the ingredients detected in this image to create a recipe with Snap-Bite AI.")
            with right_action:
                if st.button("🍳 Recipe", use_container_width=True, key="image_recipe_btn"):
                    try:
                        with st.spinner("Creating your recipe..."):
                            st.session_state.generated_recipe = generate_recipe_with_llm(selected_ingredients, "Create a recipe using only these detected ingredients as the main ingredients.")
                    except Exception as e:
                        st.error(f"Recipe generation failed: {e}")
    ...
    st.markdown('<div class="muted">Speak your cooking request and Snap-Bite will turn it into a recipe.</div>', unsafe_allow_html=True)
