...
div[data-testid="stHorizontalBlock"] div[data-testid="stButton"]:has(button[aria-label="Refresh this page"]) button {{
...
def get_secret(name):
    if name not in st.secrets or not str(st.secrets[name]).strip():
        raise RuntimeError(f"{name} is missing from Streamlit Secrets.")
...
COOKING_KEYWORDS = {
    "recipe","cook","cooking","meal","dish","food","ingredient","ingredients",
    "breakfast","lunch","dinner","snack","dessert","soup","salad","pasta",
    "chicken","beef","fish","salmon","shrimp","egg","eggs","rice","bread",
    "tomato","onion","potato","cheese","milk","flour","avocado","garlic",
    "healthy","calorie","calories","serving","servings","bake","boil","fry",
    "grill","oven","air fryer","quick","easy","vegetarian","vegan"
}

def is_cooking_request(text):
    q = (text or "").lower().strip()
    return not q or any(k in q for k in COOKING_KEYWORDS)
...
def generate_recipe_with_llm(ingredients=None, user_prompt=""):
    from openai import OpenAI

    if user_prompt.strip() and not is_cooking_request(user_prompt):
        return "OUT_OF_SCOPE: I can only help with cooking and recipe requests."

    api_key = get_secret("OPEN_ROUTER_KEY_DAY")
    client = OpenAI(base_url="https://openrouter.ai/api/v1", api_key=api_key)

    ingredient_text = ", ".join(ingredients or []) or "Use ingredients mentioned in the user's request."
    request_text = user_prompt.strip() or "Create a recipe using these ingredients."

    prompt = f"""
You are Snap-Bite, a recipe-only AI assistant.
Available ingredients: {ingredient_text}
User request: {request_text}

ONLY create cooking recipes. If the request is unrelated to food or cooking, return:
OUT_OF_SCOPE: I can only help with cooking and recipe requests.

Create ONE realistic recipe. Use the available ingredients as the main ingredients.
Never use pork or pork meat. Pantry basics are allowed.
...
Give a clear quantity for every ingredient.
Number the instruction steps.
CALORIES must be an estimate per ONE serving.
Do not provide protein, carbs, fat, or other nutrition values.
"""
    result = client.chat.completions.create(
        model="cohere/north-mini-code:free",
        messages=[
            {"role": "system", "content": "You are a recipe-only cooking assistant. Never answer non-cooking questions."},
...
def parse_generated_recipe(text):
    ...
    if text.strip().upper().startswith("OUT_OF_SCOPE:"):
        data["out_of_scope"] = text.split(":", 1)[1].strip()
        return data
    if text.strip().upper().startswith("OUT_OF_SCOPE:"):
        data["out_of_scope"] = text.split(":", 1)[1].strip()
        return data
    ...
    for raw in text.splitlines():
        ...
        upper = line.upper()
        matched = False
        for key in ["RECIPE NAME", "CATEGORY", "CUISINE", "SERVINGS", "CALORIES", "PROTEIN", "CARBS", "FAT", "PREP TIME", "COOK TIME", "DESCRIPTION", "INGREDIENTS", "INSTRUCTIONS", "SERVING SUGGESTION"]:
            prefix = key + ":"
            if upper.startswith(prefix):
                ...
                matched = True
                break
        if not matched and section in ("ingredients", "instructions"):
...
def generated_recipe_image(data):
    name = data.get("recipe_name", "").lower()
    best = None
    best_score = 0
    for recipe in recipes:
        score = sum(1 for token in recipe_tokens(recipe) if token in name)
        if score > best_score:
            best_score, best = score, recipe
    if best:
        return find_recipe_image(best["id"])
    return None


def render_generated_recipe(text):
    data = parse_generated_recipe(text)

    if data.get("out_of_scope"):
        st.warning(data["out_of_scope"])
        return

    st.markdown(
        f'<div class="section-title">{data.get("recipe_name", "Your Recipe")} 🍴</div>',
        unsafe_allow_html=True,
    )
    ...
    for key, cls, icon in [
        ...
        if data.get(key):
            suffix = " kcal / serving" if key == "calories" else (" min" if key == "prep_time" else "")
            tags.append(f'<span class="tag {cls}">{icon}{data[key]}{suffix}</span>')
    st.markdown("".join(tags), unsafe_allow_html=True)

    # Servings control: ingredient quantities are scaled automatically.
    match = re.search(r"\d+(?:\.\d+)?", data.get("servings", "1"))
    base_servings = max(1, int(float(match.group())) if match else 1)
    servings = st.number_input(
        ...
        value=base_servings,
        step=1,
        key=f"generated_servings_{st.session_state.get('generated_recipe_version', 0)}",
    )
    ...
    with left:
        img = generated_recipe_image(data)
        if img:
            render_image(img)
        else:
            st.markdown(
                '<div class="card" style="font-size:5rem;text-align:center;padding:2rem">🍽️</div>',
                unsafe_allow_html=True,
            )
        ...
        for ing in data.get("ingredients", []):
            st.markdown(
                f'<div class="ingredient-row">{scale_ingredient(ing, factor)}</div>',
                unsafe_allow_html=True,
            )
    ...
    with right:
        ...
        st.markdown(
            f'<div class="metric-card"><div class="metric-num">{data.get("calories", "—")}</div>'
            f'<div class="metric-label">Calories / serving</div></div>',
            unsafe_allow_html=True,
        )

        if data.get("description"):
            st.markdown("### About this recipe")
            st.write(data["description"])

        st.markdown("### 👨‍🍳 Instructions")
        for i, step in enumerate(data.get("instructions", []), 1):
            st.markdown(
                f'<div class="step-row"><b>{i}.</b> {step}</div>',
                unsafe_allow_html=True,
            )

        if data.get("serving_suggestion"):
            st.markdown("### 🍽️ Serving Suggestion")
            st.write(data["serving_suggestion"])
...
if "generated_recipe_version" not in st.session_state:
    st.session_state.generated_recipe_version = 0
...
# Global refresh button — available at the top of every page.
refresh_left, refresh_right = st.columns([12, 1])
with refresh_right:
    if st.button("↻", help="Refresh this page", key="global_refresh"):
        for key in [
            ...
            "voice_prompt",
            "generate_recipe",
            "generated_recipe_version",
        ]:
            if key in st.session_state:
                if key in ("detected_ingredients",):
                    st.session_state[key] = []
                elif key in ("selected_recipe_id", "generated_recipe", "voice_prompt"):
                    st.session_state[key] = None
                elif key == "generated_recipe_version":
...
if page == "🏠 Welcome":
    ...
    st.markdown('<div class="muted">Choose how you want to add your ingredients.</div>', unsafe_allow_html=True)
    ...
    if st.session_state.camera_mode == "camera":
        camera = st.camera_input("Take a photo", key="camera_capture")
        selected = camera

    elif st.session_state.camera_mode == "upload":
    ...
    if selected:
        ...
        if not MODEL_FILE.exists():
            ...
            try:
                ...
                for resu in results:
                    if resu.boxes is None:
                        continue
                    for cls_id in resu.boxes.cls.tolist():
                ...
                if detected:
                    tags = " ".join(
                        f'<span class="tag tag-g">{x}</span>' for x in detected
                    )
                    st.markdown(tags, unsafe_allow_html=True)

                    st.markdown("")
                    if st.button("🍳 Get Recipe", use_container_width=True, key="image_get_recipe"):
                        try:
                            with st.spinner("Creating your recipe..."):
                                st.session_state.generated_recipe = generate_recipe_with_llm(
                                    detected,
                                    "Create a recipe using only these detected ingredients as the main ingredients."
                                )
                                st.session_state.generated_recipe_version += 1
                            st.success("Recipe generated!")
                        except Exception as e:
                            st.error(f"Recipe generation failed: {e}")

                    if st.session_state.generated_recipe:
                        render_generated_recipe(st.session_state.generated_recipe)
                else:
                    st.warning("No ingredients were detected. Try another image.")
    ...
    st.markdown('<div class="muted">Speak your ingredients or cooking request and Snap-Bite will create a recipe.</div>', unsafe_allow_html=True)

    audio = st.audio_input("Record your cooking request")
    ...
    if audio:
        try:
            ...
            with st.spinner("Creating your recipe with AI..."):
                st.session_state.generated_recipe = generate_recipe_with_llm([], transcript)
                st.session_state.generated_recipe_version += 1

            st.success("Recipe generated from your voice!")
        except Exception as e:
    ...
    st.markdown('<div class="muted">Write your ingredients or tell Snap-Bite what you want to cook.</div>', unsafe_allow_html=True)

    text_prompt = st.text_area(
        "Your cooking request",
        placeholder="Example: chicken, tomato and onion. Make me a healthy dinner.",
        height=140,
        key="text_recipe_prompt",
    )

    if st.button("🍳 Generate Recipe", use_container_width=True, key="text_generate_recipe"):
        if not text_prompt.strip():
            st.warning("Please write your ingredients or cooking request first.")
        else:
            try:
                with st.spinner("Creating your recipe with AI..."):
                    st.session_state.generated_recipe = generate_recipe_with_llm([], text_prompt)
                    st.session_state.generated_recipe_version += 1
                st.success("Recipe generated!")
            except Exception as e:
                st.error(f"Recipe generation failed: {e}")
