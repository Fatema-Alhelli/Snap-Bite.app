    ...
    if selected:
        ...
        if selected_ingredients:
            ...
            if st.session_state.generate_recipe:
                st.markdown("### 🤖 Recipe Request")
                prompt_preview = (
                    "Create a recipe using these detected ingredients: "
                    + ", ".join(selected_ingredients)
                )
                if recipe_prompt.strip():
                    prompt_preview += f"\n\nUser instructions: {recipe_prompt.strip()}"
                st.code(prompt_preview, language="text")
                st.caption("Next step: connect this request to the LLM API to generate the final recipe, calories, and macros.")
