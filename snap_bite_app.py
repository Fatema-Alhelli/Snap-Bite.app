...
def generate_recipe_with_llm(ingredients, user_prompt=""):
    ...
    instruction = user_prompt.strip() if user_prompt else "No special instructions. Create a practical recipe using the detected ingredients."
...
User's optional request:
{instruction}

Create one realistic recipe that mainly uses the detected ingredients.
...
Give estimated nutrition for one serving. Keep the recipe clear and practical.
"""
...
if page == "🏠 Welcome":
    ...
    if selected:
        ...
        if selected_ingredients:
            ...
            st.markdown("### 🍳 Create a Recipe")
            recipe_prompt = st.text_area(
                "Additional instructions (Optional)",
                placeholder="e.g. Make it healthy, high-protein, or under 500 calories.",
                height=90,
                key="recipe_prompt",
            )

            if st.button("🍳 Generate Recipe", use_container_width=True, key="generate_recipe_btn"):
                st.session_state.generate_recipe = True
                st.session_state.generated_recipe = None
                st.info("Recipe generation is ready. Connect the LLM API to generate the recipe.")
    ...
    if st.session_state.voice_prompt:
        ...
        st.caption("This text will be used as the optional cooking instruction when we connect the LLM.")
    st.markdown('</div>', unsafe_allow_html=True)
